    ...
    <div className="w-full bg-surface text-on-surface font-body-md overflow-x-hidden pt-24 pb-section-gap">
      ...
      <div className="max-w-[900px] mx-auto px-margin-mobile md:px-0">
        ...
        <header className="text-center mb-20 space-y-6">
          ...
          <h2 className="font-display-lg text-4xl md:text-5xl font-extrabold text-primary">
            {settings?.letterTitle || "Thư Cảm Ơn Tập Thể"}
          </h2>
          <div className="flex items-center justify-center gap-4">
            <span className="h-px w-12 bg-outline-variant"></span>
            <p className="font-label-sm text-xs font-bold uppercase tracking-[0.2em] text-on-surface-variant">
              {settings?.letterSubtitle || "Ban Tổ Chức Chiến Dịch Tiếp Sức Mùa Thi 2026"}
            </p>
            <span className="h-px w-12 bg-outline-variant"></span>
          </div>
          <p className="font-body-md text-sm italic text-on-surface-variant">
            {settings?.letterDate || "Hà Nội, ngày 15 tháng 07 năm 2026"}
          </p>
        </header>
        ...
        <article className="glass-panel rounded-[2rem] p-10 md:p-20 relative overflow-hidden border border-white/60 shadow-xl">
          ...
          <div className="space-y-8 relative z-10 text-on-surface text-lg leading-relaxed text-left">
            <p className="drop-cap">
              {settings?.letterContent1 || "Thân gửi tất cả các bạn tình nguyện viên, những \"người hùng thầm lặng\" của mùa hè rực rỡ năm nay. Khi những tiếng trống trường cuối cùng đã lắng lại và cánh cửa đại học đang dần mở ra cho hàng triệu thí sinh, chúng tôi - Ban Tổ Chức - mới thực sự có một khoảng lặng để viết những dòng tri ân sâu sắc nhất gửi đến các bạn."}
            </p>
            <p>
              {settings?.letterContent2 || "Suốt chặng đường vừa qua, chúng ta đã cùng nhau vượt qua cái nóng oi ả của mùa hè, những cơn mưa rào bất chợt, và cả những áp lực vô hình từ trách nhiệm. Các bạn đã không quản ngại gian khó, có mặt tại mọi điểm trường từ sáng sớm tinh sương đến khi phố xá đã lên đèn."}
            </p>

            {/* Impact Summary Cards */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-12">
              <div className="p-6 bg-white/60 backdrop-blur-md rounded-2xl text-center border border-white shadow-sm">
                <h4 className="font-display-lg text-4xl font-extrabold text-primary mb-1">
                  {settings?.letterStat1Val || "120K+"}
                </h4>
                <p className="font-label-sm text-xs text-on-surface-variant font-bold">
                  {settings?.letterStat1Lbl || "Thí sinh được hỗ trợ"}
                </p>
              </div>
              <div className="p-6 bg-white/60 backdrop-blur-md rounded-2xl text-center border border-white shadow-sm">
                <h4 className="font-display-lg text-4xl font-extrabold text-secondary mb-1">
                  {settings?.letterStat2Val || "500+"}
                </h4>
                <p className="font-label-sm text-xs text-on-surface-variant font-bold">
                  {settings?.letterStat2Lbl || "Điểm thi an toàn"}
                </p>
              </div>
              <div className="p-6 bg-white/60 backdrop-blur-md rounded-2xl text-center border border-white shadow-sm">
                <h4 className="font-display-lg text-4xl font-extrabold text-tertiary mb-1">
                  {settings?.letterStat3Val || "1.5M"}
                </h4>
                <p className="font-label-sm text-xs text-on-surface-variant font-bold">
                  {settings?.letterStat3Lbl || "Bữa ăn & nước uống"}
                </p>
              </div>
            </div>

            <p className="whitespace-pre-line">
              {settings?.letterContent3 || "Giá trị của các bạn không chỉ nằm ở những con số ấn tượng phía trên, mà còn ở nụ cười khích lệ dành cho những thí sinh đang lo lắng, ở sự chu đáo khi hướng dẫn phụ huynh chỗ nghỉ ngơi, hay cái nắm tay tiếp thêm động lực cho các sĩ tử bước vào phòng thi. Đó là những khoảnh khắc đẹp nhất của tuổi trẻ - một tuổi trẻ cống hiến và tràn đầy lòng nhân ái.\nThay mặt Ban Tổ Chức và Trung ương Đoàn TNCS Hồ Chí Minh, tôi xin gửi lời cảm ơn chân thành và niềm tự hào lớn lao đến từng cá nhân đã góp phần làm nên thành công của \"Tiếp Sức Mùa Thi 2026\". Chúc các bạn luôn giữ vững ngọn lửa nhiệt huyết này trong mọi hành trình sắp tới của cuộc đời."}
            </p>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="py-4 select-none">
                <img
                  ...
                  src={settings?.letterSignature || "https://lh3.googleusercontent.com/aida-public/AB6AXuAm4TGg2nwCSvz2tbfn6uTTZd96PpdwTv8kfW7zERoQkVTTtvjiof--MpYX2Uu0sOAeCmWXinNCVc3raF7meTFvpOZIEB2OvLBv0lNJ1iOaqUGKIDnn5rrNOTlwin0qO_AkMFdtwReeCXC5bfJt-7q0Kb_VRE-bDEymUb31qswEVdUvRpBybXd43xahXeHRHuLII3hAlmecYcCEaauTmfmpiT8yUWaUK39ERmFgcNOI1k2X2s1Z5Om4"}
                />
              </div>
              <div>
                <p className="font-headline-md text-xl font-bold text-primary">
                  {settings?.letterSignerName || "Nguyễn Minh Hoàng"}
                </p>
                <p className="text-xs text-on-surface-variant font-semibold">
                  {settings?.letterSignerRole || "Trưởng Ban Tổ Chức Chương Trình"}
                </p>
                <p className="text-xs text-on-surface-variant">
                  {settings?.letterSignerOrg || "Trung ương Đoàn TNCS Hồ Chí Minh"}
                </p>
              </div>
            </div>
          </footer>
